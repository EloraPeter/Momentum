...
// Deep link handler for opening specific skills
window.handleDeepLink = function () {
    const urlParams = new URLSearchParams(window.location.search);
    const skillId = urlParams.get('openSkill');

    if (skillId && window.viewSkill) {
        setTimeout(() => {
            window.viewSkill(skillId);
            showToast('Opening skill...', 'info');
        }, 500);
    }
};
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', window.handleDeepLink);
} else {
    window.handleDeepLink();
}

// Generate deep link for sharing
window.generateDeepLink = function (skillId) {
    const baseUrl = window.location.origin + window.location.pathname;
    return `${baseUrl}?openSkill=${skillId}`;
};

// Copy deep link to clipboard
window.copySkillDeepLink = function (skillId) {
    const link = window.generateDeepLink(skillId);
    navigator.clipboard.writeText(link);
    showToast('Skill link copied! Share it with friends 📋');
};
...
// Add this to your script.js to enable backup/restore functionality
window.updateSkills = function (newSkills) {
    skills = newSkills;
    saveState();
    renderDashboard();
};
